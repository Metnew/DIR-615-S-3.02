    ...
    return (
      (e.parse = function(r) {
        ...
        (p.source = r),
          (p.browser = i(r, t)),
          l(p.browser)
            ? ((p.browser.name = m(p.browser)),
              (p.browser.version = d(p.browser)))
            : (p.browser = {}),
          (p.os = i(r, c)),
          l(p.os)
            ? ((p.os.name = m(p.os)), (p.os.version = d(p.os)))
            : (p.os = {}),
          (p.device = i(r, f)),
          l(p.device)
            ? ((p.device.name = m(p.device)), (p.device.version = d(p.device)))
            : (p.device = { tablet: !1, family: 'Other' });
        var s = {};
        return (
          ...
          (p.device.type =
            'Spider' === p.browser.family
              ? 'Spider'
              : p.browser.tablet || p.os.tablet || p.device.tablet
                ? 'Tablet'
                : s.hasOwnProperty(p.browser.family)
                  ? 'Mobile'
                  : 'Desktop'),
          (p.device.manufacturer =
            p.browser.man || p.os.man || p.device.man || null),
          n([p.browser, p.os, p.device], ['tablet', 'man']),
          p
        );
      }),
      e
